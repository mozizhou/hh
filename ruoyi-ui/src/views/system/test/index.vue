<template>
  <div class="app-container">
        ...
        <el-col :span="6">
          <div class="grid-content flex-text-col">
            <el-form-item label="测试目的" prop="testPurpose">
              <el-input v-model="queryParams.testPurpose" placeholder="请输入内容"/>
            </el-form-item>
          </div>
        </el-col>
      </el-row>
    ...
    <el-dialog :title="title" :visible.sync="open" width="780px" append-to-body onclose="onClickClose()" v-dialogDrag :close-on-click-modal="false">
      <el-form ref="form" :model="form" :rules="rules" label-width="80px" :disabled="disable">
        <el-row>
          <el-col :span="24">
            <el-form-item label="日期" prop="createDate">
              <el-date-picker v-model="form.createDate" value-format="yyyy/MM/dd HH:mm:ss"
          ...
          <el-col :span="24">
            <el-form-item label="试验条件" prop="testConditions">
              <el-input v-model="form.testConditions" placeholder="请输入试验条件"/>
            </el-form-item>
          </el-col>
          <el-col :span="24">
            <el-form-item label="仪器设备">
              <el-input v-model="form.equipment" placeholder="请输入仪器设备"/>
            </el-form-item>
          </el-col>
          <el-col :span="24">
            <el-form-item label="操作步骤">
              <el-input type="textarea" :rows="2" v-model="form.procedures" placeholder="请输入操作步骤"/>
            </el-form-item>
          </el-col>
        </el-row>
        <el-row>
          <el-col :span="24">
            <el-form-item label="操作、现象、数据">
              <el-input type="textarea" :rows="2" v-model="form.operateAndPhenomenonAndData"
                        placeholder="请输入操作步骤"/>
            </el-form-item>
          </el-col>
        </el-row>
        <el-row>
          <el-col :span="18">
            <el-form-item label="实验结果">
              <el-input type="textarea" :rows="2" v-model="form.testResult" placeholder="请输入实验结果"/>
            </el-form-item>
          </el-col>
          <el-col :span="6">
            <el-form-item label="试验人">
              <el-input v-model="form.experimenters" placeholder="请输入试验人"/>
            </el-form-item>
          </el-col>
        </el-row>
      </el-form>
...
export default {
  ...
  methods: {
    ...
    exportWord() {
      const _this = this
      JSZipUtils.getBinaryContent('/static/项目测试.docx', function(error, content) {
        // 抛出异常
        if (error) {
          throw error
        }
    ...
    handleChange() {
      ...
      getById({projectId: this.form.projectId[1]}).then(res => {
        if (res.data[0]) {
    ...
    getList() {
      ...
      getTestList(this.addDateRange(this.queryParams, this.dateRange)).then(response => {
    ...
    handleAdd() {
      ...
      if (this.form.projectId) {
        this.form.projectId = [this.form.company, this.form.projectId]
      }
      this.disable = false
      this.open = true
      this.title = "添加测试记录"
      ...
      this.$refs["form"].validate(valid => {
        if (valid) {
          if (this.form.id != undefined && this.form.id != "") {
            ...
            updateTestList(this.form).then(res => {
            ...
            this.form.company = this.form.projectId[0]
            this.form.projectId = this.form.projectId[1]
            insertTestList(this.form).then(res => {
    ...
    handleDelete(row) {
      const id = row.id
      this.$modal.confirm('是否确认删除测试记录编号为"' + id + '"的数据项？').then(function () {
        return removeTestList(id)
      }).then(() => {
        this.getList()
...
export function insertTestList(data) {
  return request({
    url: '/system/test',
    method: 'post',
    data: data
...
export function updateTestList(data) {
  return request({
    url: '/system/test',
    method: 'put',
    data: data
...
export function removeTestList(id) {
  return request({
    url: '/system/test/' + parseStrEmpty(id),
    method: 'delete'
  })
}
